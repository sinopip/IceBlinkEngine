...
namespace IceBlink
{
    ...
    public class PathfinderMainArea
    {
        Point[] _movements; // Movements is an array of various directions.
        Game pf_game;
        ...
        public PathfinderMainArea(Game g)
        {
            pf_game = g;
            _squares = new CompleteSquare[pf_game.currentArea.MapSizeInSquares.Width, pf_game.currentArea.MapSizeInSquares.Height];
            InitMovements(4);
            ClearSquares();
        }
        public void InitMovements(int movementCount)
        ...
        public void Pathfind(PC pc)
        {
            ...
            while (true)
            {
                ...
                foreach (Point mainPoint in AllSquares(pf_game))
                {
                     ...
                     * 
                     * */
                    if (SquareOpenForPlayer(x, y, pc))
                    {
                        int passHere = _squares[x, y].DistanceSteps;

                        foreach (Point movePoint in ValidMovesForPlayer(x, y, pc))
                        {
                            int newX = movePoint.X;
                            int newY = movePoint.Y;
                            int newPass = passHere + 1;
        ...
        public static bool ValidCoordinates(int x, int y, Game pf_game)
        {
            ...
            //if (x > (pf_game.currentArea.MapSizeInSquares.Width - 2))
            if (x > (pf_game.currentArea.MapSizeInSquares.Width - 1))  //YN1_20131027
            {
                return false;
            }
            //if (y > (pf_game.currentArea.MapSizeInSquares.Height - 2))
            if (y > (pf_game.currentArea.MapSizeInSquares.Height - 1)) //YN1_20131027
            {
                return false;
            }
            return true;
        ...
        private bool SquareOpen(int x, int y, Creature crt)
        {
            ...
            if (crt.Size == 1)
            {
                //if ((x > pf_game.currentArea.MapSizeInSquares.Width - 2) || (y > pf_game.currentArea.MapSizeInSquares.Height - 2))
                if ((x > pf_game.currentArea.MapSizeInSquares.Width - 1) || (y > pf_game.currentArea.MapSizeInSquares.Height - 1)) //YN1_20131027
                {
                    return false;
                }
                if ((_squares[x, y].ContentCode == SquareContent.Empty) || (_squares[x, y].ContentCode == SquareContent.Monster) || (_squares[x, y].ContentCode == SquareContent.Hero))
            ...
            if (crt.Size == 2)
            {
                //if ((x > pf_game.currentArea.MapSizeInSquares.Width - 3) || (y > pf_game.currentArea.MapSizeInSquares.Height - 3))
                if ((x > pf_game.currentArea.MapSizeInSquares.Width - 2) || (y > pf_game.currentArea.MapSizeInSquares.Height - 2)) //YN1_20131027
                {
                    return false;
                }
                if ((_squares[x, y].ContentCode == SquareContent.Empty) || (_squares[x, y].ContentCode == SquareContent.Monster) || (_squares[x, y].ContentCode == SquareContent.Hero))
            ...
            if (crt.Size == 3)
            {
                //if ((x > pf_game.currentArea.MapSizeInSquares.Width - 4) || (y > pf_game.currentArea.MapSizeInSquares.Height - 4))
                if ((x > pf_game.currentArea.MapSizeInSquares.Width - 3) || (y > pf_game.currentArea.MapSizeInSquares.Height - 3)) //YN1_20131027 
                {
                    return false;
                }
                if ((_squares[x + 0, y + 0].ContentCode == SquareContent.Empty) || (_squares[x + 0, y + 0].ContentCode == SquareContent.Monster) || (_squares[x + 0, y + 0].ContentCode == SquareContent.Hero))
        ...
        private bool SquareOpenForPlayer(int x, int y, PC pc)
        {
            /*
             * 
             * A square is open if it is not a wall.
        ...
        public static IEnumerable<Point> AllSquares(Game pf_game)
        {
             ...
             * 
             * */
            for (int x = 0; x <= (pf_game.currentArea.MapSizeInSquares.Width - 1); x++)
            {
                for (int y = 0; y <= (pf_game.currentArea.MapSizeInSquares.Height - 1); y++)
                {
                    yield return new Point(x, y); //After the yield is reached, and the value is returned, the foreach calls the function again, but resumes where it left off.
                }
            }
        ...
        public IEnumerable<Point> ValidMovesForPlayer(int x, int y, PC pc)
        {
            /*
             * 
             * Return each valid square we can move to.
            ...
            foreach (Point movePoint in _movements)
            {
                ...
                int newY = y + movePoint.Y;
                //this may be the place to add the size check stuff
                if (ValidCoordinates(newX, newY, pf_game) && SquareOpenForPlayer(newX, newY, pc))
                {
                    yield return new Point(newX, newY);
                }
            }
