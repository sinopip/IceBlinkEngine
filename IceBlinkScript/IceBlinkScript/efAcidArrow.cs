    ...
    public class IceBlinkScript
    {
        public void Script(ScriptFunctions sf, string p1, string p2, string p3, string p4)
        {
            ...
            if (sf.MainMapScriptCall) //the script was called from a main map
            {
                ...
                Form1 c = sf.frm;
                float resist = (float)(1f - ((float)source.DamageTypeResistanceTotalAcid / 100f));
                ...
                c.logText(" is burned from acid for ", Color.Silver);
            ...
            else //the script was called from a combat map
            {
                ...
                else if (sf.CombatSource is Creature)
                {
                    ...
                    Combat c = sf.frm.currentCombat;
                    float resist = (float)(1f - ((float)source.DamageTypeResistanceValueAcid / 100f));
                    float damage = 2 * sf.gm.Random(4);
                    int acidDam = (int)(damage * resist);
                    if (sf.frm.debugMode) { c.logText("resist = " + resist.ToString() + " damage = " + damage.ToString() + " acidDam = " + acidDam.ToString(), Color.Silver); }
                    c.logText(source.Name, Color.Blue);
