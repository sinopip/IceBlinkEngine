...
namespace IceBlink
{
    public class IceBlinkScript
    {
        public void Script(ScriptFunctions sf, string p1, string p2, string p3, string p4)
        {
            ...
            if (pc.HP <= -20)
            {
                IBMessageBox.Show(sf.gm, "Can't haste a dead character!");
            }
            else
            {
                IBMessageBox.Show(sf.gm, name + " is now hasted for 3 rounds");
                int numberOfRounds = 3;
                Effect ef = sf.gm.module.ModuleEffectsList.getEffectByTag("haste").DeepCopy();
