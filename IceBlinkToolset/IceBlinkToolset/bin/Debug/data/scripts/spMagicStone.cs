        ...
        public void Script(ScriptFunctions sf, string p1, string p2, string p3, string p4)
        {
            ...
            if (sf.CombatSource is PC)
            {
                ...
                for (int i = 0; i < numberOfBolts; i++)
                {
                    ...
                    float damage = 1 * sf.gm.Random(3) + 1;
                    int stoneDam = (int)(damage * resist);
                    if (sf.frm.debugMode) { c.logText("resist = " + resist.ToString() + " damage = " + damage.ToString() + " stoneDam = " + stoneDam.ToString(), Color.Silver); }
                    c.logText(source.Name, Color.Blue);
                    c.logText(" attacks ", Color.Black);
                    c.logText(target.Name, Color.LightGray);
                    c.logText(" with a Magic Stone and HITS for ", Color.Black);
                    c.logText(damage.ToString(), Color.Lime);
                    c.logText(" point(s) of magic damage", Color.Black);
