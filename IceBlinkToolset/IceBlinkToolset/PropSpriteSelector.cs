...
{
    public partial class PropSpriteSelector : Form
    {
        private Game game;
        ...
        public PropSpriteSelector(Game g, Module m)
        {
            InitializeComponent();
            game = g;
            mod = m;
        ...
        private void loadSpriteList()
        {
            spriteList.Clear();
            string jobDir = "";
            jobDir = game.mainDirectory + "\\modules\\" + mod.ModuleFolderName + "\\graphics\\sprites\\props";
            foreach (string f in Directory.GetFiles(jobDir, "*.spt"))
            {
                ...
                catch (Exception ex)
                {
                    ...
                    game.errorLog("Failed to open the following file from the module's props folder: " + filename + " ...(image may be wrong size or corrupted): " + Environment.NewLine + ex.ToString() + Environment.NewLine + Environment.NewLine);
                }
            }
            ...
            jobDir = game.mainDirectory + "\\data\\graphics\\sprites\\props";
            foreach (string f in Directory.GetFiles(jobDir, "*.spt"))
            {
                string filename = Path.GetFileName(f);
                try
                ...
                catch (Exception ex)
                {
                    ...
                    game.errorLog("Failed to open the following file from the ...\\data\\graphics\\sprites\\props folder: " + filename + " ...(image may be wrong size or corrupted): " + Environment.NewLine + ex.ToString() + Environment.NewLine + Environment.NewLine);
                }
            }
        }
