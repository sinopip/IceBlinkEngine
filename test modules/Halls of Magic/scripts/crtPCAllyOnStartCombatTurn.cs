...
namespace IceBlink
{
    public class IceBlinkScript
    {
        ...
        {;
            if (crt.Intelligence > 13) //will attack Creature with lowest HP
            {
                Creature crttarget = sf.TargetCreatureWithLowestHP(true); //(true) means that will ignore Creatures in Stealth Mode                
                sf.CombatTarget = crttarget; //this will set the creature's target to be the PC with the fewest HP
            }
            else //will attack the closest Creature
            {
                Creature crttarget = sf.TargetClosestCreature(true);
                sf.CombatTarget = crttarget;
            }
        }
        public void DebuffCaster(ScriptFunctions sf, Combat c, Creature crt)
        {
            ...
            if (crt.Intelligence > 13)
            {
                ...
                {
                    Creature crttarget = sf.TargetClosestCreatureNotHeld(true); //(true) means that will ignore PCs in Stealth Mode
                    if (crt != null)
                    {
                        if (sf.SpellToCast.SpellTargetType == TargetType.PointLocation)
                        //if (sf.SpellToCast.TargetIsPointLocation)
                        {
                            sf.CombatTarget = crttarget.CombatLocation; //we are using a Point type because the selected spell is looking for a Point type (square location)
                        }
                    ...
                    {
                        crttarget = sf.TargetClosestCreature(true);
                        sf.CombatTarget = crttarget;
                ...
                for (int i = 0; i < 10; i++)
                {
                    ...
                    if (sp != null)
                    {
                        ...
                        if ((sp.CostSP <= crt.SP) && (sp.SpellEffectType == Spell.EffectType.Debuff))
                        {
                            ...
                            Creature crttarget = sf.TargetClosestCreature(true); //(true) means that will ignore PCs in Stealth Mode
                            if (sf.SpellToCast.SpellTargetType == TargetType.PointLocation)
                            //if (sf.SpellToCast.TargetIsPointLocation)
                            {
                                sf.CombatTarget = crttarget.CombatLocation; //we are using a Point type because the selected spell is looking for a Point type (square location)
        ...
        public void Healer(ScriptFunctions sf, Combat c, Creature crt)
        {
            ...
            PC targetCrt = sf.GetPCWithLowestHP();
            if (targetCrt != null)
            {
        ...
        public void GeneralCaster(ScriptFunctions sf, Combat c, Creature crt)
        {
            ...
            if (sf.SpellToCast == null) //didn't find a spell that matched the criteria so use attack instead
            {
                sf.CombatTarget = sf.TargetClosestCreature(true);
                sf.ActionToTake = ChosenAction.MeleeRangedAttack;
            }
        }
